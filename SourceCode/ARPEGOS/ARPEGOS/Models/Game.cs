...
namespace ARPEGOS_Test.Models
{
    ...
    public class Game
    {
        ...
        public static string ProjectPath { get; internal set; } = "D:/Arpegos-Test/";
        ...
        public Game(string GameName, string GameVersion)
        {
            ...
            CurrentCharacterName = "TestCharacter";
            CurrentCharacterFile = Path.Combine(CharacterFolder, CurrentCharacterName + ".owl");
            CurrentCharacterContext = "http://arpegos-project.org/games/" + CurrentGameName + "/characters/" + CurrentCharacterName + "#";
            string CurrentGamePrefix = string.Concat(Regex.Matches(CurrentGameName, "[A-Z]").OfType<Match>().Select(match => match.Value)).ToLower();
        ...
        internal string CheckGeneralCost(string stage)
        {
            ...
            while (FilterCounter > 1)
            {
                if (wordCounter > 0)
                {
                    ...
                    for (int i = 1; i < wordCounter - 1; ++i)
                        SubjectFactName += StageWords.ElementAtOrDefault(i) + "_";
                    SubjectFactName += StageWords.LastOrDefault();

                    RDFOntologyClass SubjectFact = GameOntology.Model.ClassModel.SelectClass(CurrentGameContext + SubjectFactName);
                    if (SubjectFact != null)
                    {
                        IEnumerable<RDFOntologyTaxonomyEntry> SubjectFactCostAnnotations = GameOntology.Model.ClassModel.Annotations.CustomAnnotations.SelectEntriesBySubject(SubjectFact);
        ...
        internal string CheckValueListInfo(string stage)
        {
            ...
            RDFOntologyClass StageDefinitionIndividual = GameOntology.Model.ClassModel.SelectClass(CurrentGameContext + stage);
            RDFOntologyTaxonomy StageAnnotations = GameOntology.Model.ClassModel.Annotations.CustomAnnotations.SelectEntriesBySubject(StageDefinitionIndividual);
        ...
        internal dynamic GetCharacterSkills(string CharacterName)
        {
            dynamic CharacterSkills = null;
            List<string> SkillNames = new List<string> { "Habilidad", "Skill", "Aptitude" };
            List<string> CharacterFiles = Directory.EnumerateFiles(CharacterFolder).ToList();
            if (CharacterFiles.Contains(CharacterName))
            {
                ...
                foreach (string name in SkillNames)
                {
                    if (CharacterSkills != null)
                        ...
                        CharacterSkills = GetIndividualsGrouped(name, true);
                }
            }
        ...
        {
            string creationSchemeRootUri  = GameOntology.Model.ClassModel.Annotations.CustomAnnotations.SelectEntriesByPredicate(new RDFOntologyAnnotationProperty(new RDFResource(CurrentGameContext + "CreationSchemeRoot"))).Single().TaxonomySubject.ToString();
            return creationSchemeRootUri.Substring(creationSchemeRootUri.LastIndexOf('#') + 1);
        }
        /// <summary>
        ...
        internal SortedList<int, string> GetOrderedSubstages(string stage)
        {
            ...
            {
                RDFOntologyClass SubstageDefinitionClass = Program.Game.GameOntology.Model.ClassModel.SelectClass(Program.Game.CurrentGameContext + substage.Title);
                RDFOntologyTaxonomyEntry SubstageOrderEntry = Program.Game.GameOntology.Model.ClassModel.Annotations.CustomAnnotations.SelectEntriesBySubject(SubstageDefinitionClass).Where(entry => entry.TaxonomyPredicate.ToString().Contains("SubstageOrder")).SingleOrDefault();
                                            ...
                                            if (newItem == null)
                                            {
                                                for(int i = index; i < expression.Count()-1; ++i)
                                                {
                                                    ...
                                                    if (CheckDatatypeProperty(newNextElement, false) == true)
                                                    {
                                                        ...
                                                        {
                                                            RDFOntologyClass classDefinitionFact = GameOntology.Model.ClassModel.SelectClass(CurrentGameContext + itemClass);
                                                            if(classDefinitionFact != null)
                                                            {
                                                                IEnumerable<RDFOntologyTaxonomyEntry> classDefinitionAnnotations = GameOntology.Model.ClassModel.Annotations.CustomAnnotations.Where(entry => entry.TaxonomySubject == classDefinitionFact);
